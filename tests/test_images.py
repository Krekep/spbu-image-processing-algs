...
def test_picture_proccessing(path):
    image = Image.open(path)
    actual_binary = otsu_method(image)
    assert isinstance(actual_binary, list)  # Мы действительно получаем список из метода оцу
    assert all(isinstance(pixel, int) for pixel in actual_binary)  # это плоский список, каждый элемент целое число
    assert all(0 <= pixel <= 255 for pixel in actual_binary)  # каждое число не выходит за границы уровня яркости
    assert len(actual_binary) == image.width * image.height  # количество чисел (пикселей) совпадает с исходным
